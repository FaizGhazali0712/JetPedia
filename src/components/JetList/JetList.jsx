...
export default function JetList({ data = defaultJets, defaultSort = 'name', initialQuery = '', onSelect } ) {
            ...
            <div className="p-4 flex-1 flex flex-col">
              ...
              <div className="mt-4">
                <button
                  ...
                  aria-expanded={openId === jet.id}
                  className="text-sm text-[#0f2540] font-medium hover:underline"
                >
                  {openId === jet.id ? 'Hide armament' : 'View armament'}
                </button>

                {openId === jet.id && (
                  <ul className="mt-2 text-sm text-gray-700 list-disc list-inside">
                    {jet.armament?.map((a, i) => (
                      <li key={i}>{a}</li>
                    ))}
                  </ul>
                )}
